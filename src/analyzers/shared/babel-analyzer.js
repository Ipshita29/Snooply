// Shared engine for Babel-parseable languages (JavaScript, TypeScript).
// Walks the AST looking for imports/requires and package usage. The
// only thing that differs between languages is which file extensions
// to scan and which Babel parser plugins to enable - everything else
...
// (`import type { X }` and `import { type X }` land here too - a
// type-only import is still a real reference to the package)
function analyzeFile(code, dependencies, usage, parserPlugins) {
...
// Build an analyzer for a Babel-parseable language. `getParserPlugins`
